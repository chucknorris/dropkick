...
namespace dropkick.Tasks.Security.Acl
{
    ...
    public class GrantReadTask : BaseGrantPermissionTask, Task
    {
        public GrantReadTask(string path, string @group, DotNetPath dnPath)
        {
            Path = path;
            Group = group;
            _dotNetPath = dnPath;
        ...
        public string Name
        {
            get { return "Grant Read permissions to '{0}' for path '{1}'".FormatWith(Group, Path); }
        }
