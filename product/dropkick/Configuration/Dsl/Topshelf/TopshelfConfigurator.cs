...
namespace dropkick.Configuration.Dsl.Topshelf
{
        ...
        TopshelfOptions
    {
        ...
        {
            var loc = _location;
            loc = _path.ConvertUncShareToLocalPath(site, _location);

            if (site.IsLocal)
            {
                site.AddTask(new LocalTopshelfTask(_exeName, loc, _instanceName));
            }
            else
            {
                site.AddTask(new RemoteTopshelfTask(_exeName, loc, _instanceName, site));
            }
        }
    }
>>>>>>> adding path support to topshelf
}